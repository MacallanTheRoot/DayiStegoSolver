...
def analyze_pixel_data(
    pixel_data: PixelData, cancel_event: threading.Event | None = None
) -> ChiSquareAnalysis:
    ...
    frequencies = [0] * 256
    for index, value in enumerate(pixel_data.color_bytes):
        if index % 65_536 == 0:
            _check_cancelled(cancel_event)
        frequencies[value] += 1
...
def _analyze_file_sync(
    ...
    return analyze_pixel_data(pixels, cancel_event)
...
async def run_chi_square(
    ...
    del flag_pattern  # This analyzer reports a heuristic rather than flags.
        ...
        with ThreadPoolExecutor(
            ...
            try:
                analysis = await asyncio.wait_for(
                    asyncio.shield(worker), timeout=max(1.0, timeout)
                )
                ...
                logger.warning(
                    f"[-] Chi-square hesabı {timeout}s sınırını aştı yeğenim; "
                    "işçiyi temizleyip sıradakine geçtim."
                )
                return ToolResult(
                    tool_name=TOOL_NAME,
                    command=command,
                    return_code=None,
                    stdout="",
                    stderr=f"chi-square analysis exceeded {timeout}s timeout",
                    flags_found=[],
