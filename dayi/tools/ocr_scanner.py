...
def _discover_images(target: Path, workspace: Path) -> list[tuple[Path, str]]:
    ...
    for path, label in raw_candidates:
        ...
        if size <= 0 or size > MAX_IMAGE_BYTES or not _has_supported_image_magic(path):
            continue
        if total_image_bytes + size > MAX_TOTAL_IMAGE_BYTES:
            break
...
def _ocr_image_sync(
    ...
    return _safe_ocr_text(text, MAX_TEXT_PER_IMAGE)
...
def _emit_artifact(
    ...
    try:
        ...
        logger.debug(f"[ocr_scanner] Artifact bildirimi iletilemedi yeğenim: {exc}")
...
async def run_ocr_scanner(
    ...
    try:
        await asyncio.wait_for(
            asyncio.to_thread(dependencies.pytesseract.get_tesseract_version),
