"""
dayi/tools/strings.py
~~~~~~~~~~~~~~~~~~~~~~
Async runner for `strings`: extracts printable character sequences from binary files.
"""
import logging
import re
...
# Minimum string length to extract (4 is the GNU default)
MIN_LENGTH = 4
...
async def run_strings(
    ...
    """
    Run the `strings` utility on the target file and scan output for flags.
    ...
    """
    cmd = [BINARY, "-a", "-n", str(MIN_LENGTH), str(target)]
