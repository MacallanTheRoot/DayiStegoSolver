...
from dayi.scanner import ArtifactFinding, decode_base64_text, scan_artifacts
...
class DayiRunner:
    ...
    async def run_all(self) -> ScanReport:
        ...
        workspace_manager = tempfile.TemporaryDirectory(prefix="dayi_runner_")
        self._workspace = Path(workspace_manager.name)
    ...
    def _make_context(
        ...
        return PluginContext(
            ...
            progress_reporter=lambda attempted, total: self._ui_call(
                "plugin_progress", plugin_id, attempted, total
            ),
            artifact_reporter=lambda message: self._ui_call(
                "show_artifact", message
            ),
        )
    ...
    def _result_outcome(result: ToolResult) -> str:
        if result.skipped:
            if result.skip_reason.startswith("Unhandled plugin exception:"):
                return "failed"
            return "skipped"
        if result.timed_out:
            return "timed_out"
    ...
    def _attach_artifacts(self, result: ToolResult) -> None:
        ...
        for stream_name, content in (("stdout", result.stdout), ("stderr", result.stderr)):
            ...
            for finding in scan_artifacts(content, source=source):
                key = (
