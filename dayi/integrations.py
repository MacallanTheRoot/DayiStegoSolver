...
~~~~~~~~~~~~~~~~~~~~~
Real-time flag notification and auto-submission integration for Dayı v2.0.
...
# ---------------------------------------------------------------------------
# Dynamic library resolution — executed once at module import time.
# ---------------------------------------------------------------------------

_CTFSHIT_AVAILABLE = False
_AIOHTTP_AVAILABLE = False

# Tier 1: ctfshit (local sibling package — ctfshit/src/*.py)
# Import path: ctfshit.src.* (the ctfshit/ directory is a package in the repo root)
try:
    from ctfshit.src.api_client       import CTFdAPIClient as _CTFdAPIClient       # type: ignore[import]
    from ctfshit.src.flag_submitter   import FlagSubmitter as _FlagSubmitter        # type: ignore[import]
    from ctfshit.src.discord_notifier import send_flag_notification as _send_discord  # type: ignore[import]
    _CTFSHIT_AVAILABLE = True
    logger.debug("[integrations] ctfshit kütüphanesi bulundu — FlagSubmitter ve Discord notifier hazır.")
except ImportError as _ctfshit_err:
    logger.debug(f"[integrations] ctfshit bulunamadı ({_ctfshit_err}), yedek HTTP moduna geçiyorum.")

# Tier 2: aiohttp (installed separately)
try:
    import aiohttp as _aiohttp  # type: ignore[import]
    _AIOHTTP_AVAILABLE = True
    logger.debug("[integrations] aiohttp mevcut, Tier-2 fallback olarak hazır.")
except ImportError:
    logger.debug("[integrations] aiohttp da yok — urllib ile idare edeceğiz yeğenim.")
...
def _urllib_post_json(url: str, payload: dict, extra_headers: dict | None = None) -> int:
    ...
    try:
        ...
        logger.debug(f"[integrations] urllib POST failed: {exc}")
        return -1
...
class IntegrationManager:
    ...
    def notify(self, flag: str, tool_name: str) -> None:
        ...
        logger.log(
            ...
            f"CTFd'ye onaya yolladım, dayın hallediyor..."
        )
    ...
    async def _send_via_ctfshit(self, flag: str) -> None:
        ...
        if self.ctfd_url and self.ctfd_token and self.challenge_id:
            try:
                async with _CTFdAPIClient(self.ctfd_url, self.ctfd_token) as api:
                    submitter = _FlagSubmitter(api)
            ...
            if self.webhook_url:
                try:
                    embed = {
                        ...
                        "footer": {
                            "text": "Dayı Stego Solver v2.0 — Hallederiz yeğenim!"
                        },
                    }
        ...
        if self.webhook_url:
            try:
                embed = {
                    ...
                    "footer": {"text": "Dayı Stego Solver v2.0 — urllib fallback"},
                }
                status_code = await loop.run_in_executor(
